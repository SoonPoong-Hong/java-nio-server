...
// https://github.com/mihasya/sample-java-nio-server/blob/master/src/JServer.java
...
/*
 * the thread is completely unnecessary, it could all just happen in main()
 */
			...
			try {
				...
				while (this.selector.select() > 0) {
					...
					while (i.hasNext()) {
						...
						if (key.isAcceptable()) {
							// this means that a new client has hit the port our main // socket is listening on,
							// so we need to accept the connection
							// and add the new client socket to our select pool for reading a command later
							System.out.println("Accepting connection!");
							// this will be the ServerSocketChannel we initially registered with the selector in main()
							ServerSocketChannel sch = (ServerSocketChannel) key.channel();
							SocketChannel ch = sch.accept();
							ch.configureBlocking(false);
							ch.register(this.selector, SelectionKey.OP_READ);
						} else if (key.isReadable()) {
							// one of our client sockets has received a command and we're now ready to read it in
							...
							// for writing since we'll want to write something // to it // on the next go-around
							ch.register(this.selector, SelectionKey.OP_WRITE);
